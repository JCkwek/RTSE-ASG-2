...
#kwek
def estimate_current_lane(road_bounds):
    ...
    # Adjust this if the camera is not centered
    car_center_x = 135

    lane_index = round((car_center_x - x_left) / lane_w)

    lane_index = max(0, min(4, lane_index))
    # lane_index = ((car_center_x - x_left) / lane_w)
    # lane_index = int(np.clip(round(lane_index), 0, 4))
...
def processing_task():
    ...
    if front_frame is not None and id(front_frame) != last_processed_id:
        ...
        measured_lane = estimate_current_lane(road_bounds)

        if measured_lane is not None:
            with state_lock:
                shared_data['lane_history'].append(measured_lane)

                if len(shared_data['lane_history']) > 10:
                    shared_data['lane_history'].pop(0)

                stable_lane = round(
                    sum(shared_data['lane_history']) /
                    len(shared_data['lane_history'])
                )

                shared_data['measured_lane'] = stable_lane

                current_estimate = shared_data['net_lane_position']

                # Only recalibrate if estimate drifts badly
                if abs(current_estimate - stable_lane) >= 2:
                    shared_data['net_lane_position'] = stable_lane
    ...
    try:
        ...
        while is_running:
            ...
            with state_lock:
                ...
                measured_lane = shared_data.get('measured_lane', 0) #kwek
            ...
            if front_frame is not None and not display_paused:
                ...
                #kwek
                vision_lane = measured_lane + 3 
                #end kwek
                cv2.putText(display_front, f"CURRENT LANE: {current_abs_lane}", (400, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

                #kwek
                cv2.putText(display_front,f"VISION LANE: {vision_lane}",(400, 60),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,0),2)
                #end kwek
                ...
                if road_bounds is not None:
                    ...
                    cv2.line(display_front,(camera_center, draw_y - 40),(camera_center, draw_y + 40),(0, 255, 0),3)
                    #end kwek
